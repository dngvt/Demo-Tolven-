...
//		root.setAttribute( 'gridPreset', grid.getAttribute( 'gridPreset' ));
	...
	new Form.Element.Observer( $(id+"-filter"), 1, function(element, val) {checkUserInput(element, val, id, methodNameValue, methodArgsValue,menuPath,outputFormat,charWidth);} );
	if( $(id).getAttribute('filterValue') != null && $(id).getAttribute('filterValue') != "" ) {
	  filterValueChange($(id + "-filter"), $(id).getAttribute('filterValue'), id, methodNameValue, methodArgsValue ,menuPath,outputFormat,charWidth );
	}
...
function cui(element, value, id, methodName, methodArgs,menuPath,gridType,charWidth){
	if( $( id + "-filter").value == value ){
	 filterValueChange(element, value, id ,methodName, methodArgs,menuPath,gridType,charWidth);
	} else $(id).setAttribute('filterValue', $( id + "-filter").value );
}

function checkUserInput(element, value, id , methodName, methodArgs,menuPath,gridType,charWidth){
	$(id).setAttribute('filterValue', value );
	setTimeout( "cui('" + element + "', '" + value + "', '" + id+  "', '" + methodName + "', '" +  methodArgs+"', '" +  menuPath + "','"+gridType+"','"+charWidth+"')", 125);
}

function filterValueChange(element, val, id, methodName, methodArgs,menuPath,outputFormat,charWidth) {
//Tolven.Util.log(val);
	if(!menuPath)
	...
	lg.setRequestParams( {name: 'element', value: menuPath}, {name: 'filter', value: val}, {name: 'methodName', value: methodName}, {name: 'methodArgs', value: methodArgs},{name: 'gridType',value:outputFormat},{name: 'charWidth',value:charWidth}  );
//	lg.resetContents();
...
SCRIPTMAP: {
	...
	'TIMELINE'  : 'simile/timeline-api.js,simile/bundle.js,tolvenUtils.js',
