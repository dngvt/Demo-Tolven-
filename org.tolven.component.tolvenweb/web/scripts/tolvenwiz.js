...
function wizNullify( element, action ) {
 if (confirm( "Mark the document as Entered in Error?" )) {
  var myAjax = new Ajax.Request(
    'wizNullify.ajaxi',
    {
     method: 'get',
...
function wizSuspend( element, action ) {
 if (confirm( "Mark the document as Suspended?" )) {
  var myAjax = new Ajax.Request(
    'wizNullify.ajaxi',
    {
     method: 'get',
...
function wizActivate( element, action ) {
 if (confirm( "Mark the document as Active?" )) {
  var myAjax = new Ajax.Request(
    'wizNullify.ajaxi',
    {
     method: 'get',
      parameters: 'element='+element+'&action=resume',
             onFailure: function(request) {displayError(request,element);},
     		 onSuccess: function(request) {wizNullifyDone(request,element);}
    });
 }
...
function wizUpload( obj, value,isLastStep ) {
  var root = $(obj);
 // Don't upload when on the last step
 if (root.getAttribute('itemStatus')=='NEW') {
  var elementId = root.getAttribute( 'tolvenid' );
  ajaxSubmit3( root, elementId ,isLastStep);
}
}
...
function ajaxSubmit3(form, element ,isLastStep) {
 // If the form is no linger being created, don't try uploading
 var formElem = $(form);
 if(isLastStep && formElem.getAttribute('ajaxSubmit3InProgress') != null){
	 setTimeout("ajaxSubmit3(" + form + ",'" + element + "',"+isLastStep+")", 150);
 }
 ...
 if(isLastStep == true)
 queryComponents.push( 'checkValidations=true' );
...
function addTemplate(templateName, element, methodArgs)
{
 ...
 $(formId + ":computeEnable"+lArgs[1]).value = "true";
 $(formId + ":computeTemplate"+lArgs[1]).value = templateName;
 $(formId + ":computePosition"+lArgs[1]).value = lArgs[1];
 $(formId + ":computeAction"+lArgs[1]).value = 'add';
...
function removeTemplate(formId,postion,stepIndex){
 var rootForm = $(formId);
 stopAsync(formId);

 $(formId + ":computeEnable"+stepIndex).value = "true";
 $(formId + ":computeAction"+stepIndex).value = 'delete';
 $(formId + ":computePosition"+stepIndex).value = postion;
 var currentStep = 1 * rootForm.getAttribute('currentStep');
 var lastStep = 1 * rootForm.getAttribute('lastStep');
 var wipNode = rootForm.parentNode;
 ajaxSubmit4(rootForm, wipNode.id, stepIndex+1);
}
...
function wizDeleteActiveListItem( element, action,type,id ) {
	 if (confirm( "You want to delete this "+type+"?" )) {
		 $(id).hide();
		 var myAjax = new Ajax.Request(
	    'doPlaceholderTrasition.ajaxi',
	    {
	     method: 'get',
	      parameters: 'element='+element+'&action='+action,
	             onFailure: function(request) {displayError(request,element);},
