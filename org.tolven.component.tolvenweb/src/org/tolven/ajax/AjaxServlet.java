...
public class AjaxServlet extends HttpServlet {
	...
	private static void writeFormattedColumn( MSColumn msColumn, FormattedOutputWriter fow, String outputExpression,Object value, int allowedWidth ) {
		try {
			if(value != null && value.toString().length() >0){
				fow.writeExpression(outputExpression,value.toString(),allowedWidth);
			}else{
	...
	private static void writeColumn(Map<String, Object> rowData, MSColumn msColumn, FormattedOutputWriter fow, MenuLocal menuLocal, HttpServletRequest req, int allowedLength) {
	    try {
			...
			if (value==null) {
				...
				if (stringValue.length()==0) {
					...
					fow.writeEscape( stringValue ,allowedLength);
					fow.writeVerbatim("</a>");
	...
	public static void writeMenuDataRow( FormattedOutputWriter fow, MDQueryResults mdQueryResults,
		...
		for (MSColumn msColumn : msColumns) {
			if (msColumn.isVisible()) {
				...
				int allowedWidth = Math.round ((msColumn.getWidth()*16) / Float.parseFloat(charWidth))-6;
				if (outputExpression!=null) {
					writeFormattedColumn(msColumn,fow, outputExpression,rowData.get(msColumn.getHeading()),allowedWidth);
				} else {
					writeColumn( rowData, msColumn, fow, menuLocal, req,allowedWidth);
				}
				fow.writeVerbatim("</td>");
			}
		}
