...
//		TolvenLogger.info( "getElement: " + element, MenuAction.class);
	 ...
	 * Get the menudata for the requested element
	 */
	public MenuData findMenuDataItem(String path ) {
		AccountUser accountUser = TolvenRequest.getInstance().getAccountUser();
		return getMenuLocal().findMenuDataItem(accountUser.getAccount().getId(), path);
	}
...
//			TolvenLogger.info( getElement() + " children: " + childPaths, MenuAction.class);
	...
	public String getSummaryMenuDataRows( ) {
	    ...
	    //set the expression evaluator to be able to Deferred columns
	    TrimExpressionEvaluator evaluator = new TrimExpressionEvaluator();
        evaluator.addVariable("account", ms.getAccount());
        evaluator.addVariable("accountUser", TolvenRequest.getInstance().getAccountUser());
        
		for (MenuData md : mdList ) {
		    fieldGetter.setMenuData(md);
		    //incase if there is a Deferred column using the placeholder to get the display text
		    evaluator.addVariable(md.getReference().getMenuStructure().getNode(), md.getReference());
