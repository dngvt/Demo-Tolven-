...
public abstract class FormattedOutputWriter {
	...
	public void writeEscape( String val, int allowedLength ) throws IOException {
		if(val.length() > allowedLength){
			val = "<span title='"+StringEscapeUtils.escapeHtml(val)+"'>"+StringEscapeUtils.escapeHtml(val.substring(0,allowedLength))+"...</span>";
			writer.write(val);
		}else
			writeEscape(val);
	}
