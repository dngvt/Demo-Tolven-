...
public class RegisterAction extends TolvenAction {
    ...
    private String inviterUserPassword;
	...
	private boolean accountDataMigrationRequired;
    private boolean accountDataMigrationInProgress;

	private @EJB
    GatekeeperClientLocal gatekeeperClientBean;
    
    @EJB
    private ConfigChangeLocal configChangeLocal;
	...
	public String createNewAccount() throws Exception {
        ...
        try {
            if(!gatekeeperClientBean.verifyUserPassword(getUser().getLdapUID(), getInviterUserPassword().toCharArray(), realm)) {
                throw new TolvenAuthenticationException("Create account permission is denied: incorrect password", getTp().getUid(), realm);
            }
            newAccount = getAccountBean().createAccount2(getNewAccountTitle(), getNewTimeZone(), getNewAccountType());
            logger.info("Created account: " + newAccount.getId() + ", acct type " + getNewAccountType().getKnownType());
            // Note, the user automatically gets administrator permission because they are the only user on that new account.
            AccountUser au = getAccountBean().addAccountUser(newAccount, getUser(), getNow(), true, getUserPublicKey());
            logger.info("Added first admin to: " + newAccount.getId() + ", acct type " + getNewAccountType().getKnownType());
            //add systemuser(probably mdbuser to the account)
            getAccountBean().addSystemUserToAccount(au,getUserPrivateKey(), getInviterUserPassword().toCharArray());
            logger.info("Adding mdbuser to account: " + newAccount.getId());
...
//        menu.createDefaultMenuStructure( account );
		getMenuLocal().updateMenuStructure(newAccount);
        if (this.isGenDemoData()) {
        	...
        	if ("echr".equals(getNewAccountType().getKnownType())) {
	        	...
	        	control.setChrAccountId(newAccount.getId());
    ...
    public String updateAccount() {
    	...
    	getAccountBean().setManualMetadataUpdate(manualMetadataUpdate, account);
...
*/
    ...
    public String updateCertInfo() throws Exception {
        ...
        String realm = TolvenSessionWrapperFactory.getInstance().getRealm();
        if(!gatekeeperClientBean.verifyUserPassword(getTp().getUid(), getOldUserPassword().toCharArray(), realm)) {
...
*/
     ...
     */
     public String updatePrefs( ) throws Exception {
        ...
        String principal = (String) TolvenSessionWrapperFactory.getInstance().getPrincipal();
        String realm = TolvenSessionWrapperFactory.getInstance().getRealm();
        boolean passwordVerified = gatekeeperClientBean.verifyUserPassword(principal, getOldUserPassword().toCharArray(), realm);
        if(!passwordVerified) {
		...
		if (getUser() != null) {
         if(getUser().getLocale() !=null &&getUser().getLocale().length() == 0 )
          getUser().setLocale(null);
...
//				TolvenLogger.info("TYPE "+ atype);
    ...
    private String inviteUser(boolean isReinvite) {
        ...
        String realm = TolvenSessionWrapperFactory.getInstance().getRealm();
        String principal = (String) TolvenSessionWrapperFactory.getInstance().getPrincipal();
        if (!gatekeeperClientBean.existsTolvenPerson(invitedUserId, realm, principal, getInviterUserPassword().toCharArray())) {
        ...
        try {
            getAccountBean().inviteAccountUser(getAccount(), getAccountUser(), invitedUserId, getInvitedUserRealm(), getUserPrivateKey(), getInviterUserPassword().toCharArray(), getNow(), false, isReinvite);
        } catch (Exception ex) {
            TolvenSecurityException gex = TolvenSecurityException.getRootGatekeeperException(ex);
    ...
    public List<SelectItem> getRealms() {
        if (realms == null) {
            ...
            for (String realmId : gatekeeperClientBean.getRealmIds()) {
	...
	public String getInviterUserPassword() {
		return inviterUserPassword;
	}


	public void setInviterUserPassword(String inviterUserPassword) {
		this.inviterUserPassword = inviterUserPassword;
	}
	public String startAccountDataMigration(){
		try{
			AccountUser accountUser = TolvenRequest.getInstance().getAccountUser();
			List<AccountMenuStructure> msList = configChangeLocal.findMenuStructureToMigrate(accountUser.getAccount());
			for(AccountMenuStructure ams:msList){
				configChangeLocal.startDataMigration(new MigrateMenuDataMessage(ams), getInviterUserPassword().toCharArray());
			}
	...
	public boolean isAccountDataMigrationRequired(){
		AccountUser accountUser = TolvenRequest.getInstance().getAccountUser();
		boolean accountDataMigrationRequired = getConfigChangeLocal().hasMenuStructureToMigrate(accountUser.getAccount());
		return accountDataMigrationRequired;
	}
	public void setAccountDataMigrationRequired(boolean accountDataMigrationRequired) {
		this.accountDataMigrationRequired = accountDataMigrationRequired;
	}
	public boolean isAccountDataMigrationInProgress(){
		AccountUser accountUser = TolvenRequest.getInstance().getAccountUser();
		return accountUser.getAccount().getDataMigrationInProgress()==null?false:accountUser.getAccount().getDataMigrationInProgress();
	}
	public void setAccountDataMigrationInProgress(boolean accountDataMigrationInProgress) {
		this.accountDataMigrationInProgress = accountDataMigrationInProgress;
	}
	public ConfigChangeLocal getConfigChangeLocal() {
		return configChangeLocal;
	}
	public void setConfigChangeLocal(ConfigChangeLocal configChangeLocal) {
		this.configChangeLocal = configChangeLocal;
	}
}
