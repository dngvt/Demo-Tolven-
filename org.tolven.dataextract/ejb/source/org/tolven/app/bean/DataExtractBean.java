...
/*            selectString.append( "DISTINCT md.id as pk" );
		...
		while(iter.hasNext()) {
			...
			if(md!=null) {
				...
				if(md.getParent02()!=null) {
					...
					DataQueryResults newDq = this.setupQuery(path,TolvenRequest.getInstance().getAccountUser());
	...
	@Override
	public void streamResultsSQL(Writer out, String queryString) throws IOException {
		AccountUser accountUser = TolvenRequest.getInstance().getAccountUser();
		Query query = em.createQuery( queryString );
		query.setParameter("account", accountUser.getAccount() );
		List<Object> results = query.getResultList();
		if(results != null && results.size() >0){
			if(results.get(0) instanceof MenuData){
				MenuData md = (MenuData)results.get(0);
		    	...
		    	try {
		            ...
		            xmlStreamWriter.writeStartDocument("UTF-8", "1.0");
		            xmlStreamWriter.writeStartElement("results");
		            xmlStreamWriter.writeAttribute("path", md.getPath());
		            xmlStreamWriter.writeAttribute("account", String.valueOf(md.getAccount().getId()));
		            xmlStreamWriter.writeAttribute("database", getTolvenPropertiesBean().getProperty("tolven.repository.oid"));
					xmlStreamWriter.writeAttribute("count", Long.toString(results.size())); 
				
					for(Object result:results){
						MenuData item = (MenuData)result;
						xmlStreamWriter.writeStartElement("result");
						xmlStreamWriter.writeStartElement("id");
						xmlStreamWriter.writeCharacters(String.valueOf(item.getId()));
						xmlStreamWriter.writeEndElement();
						if(item.getReference() != null){
							xmlStreamWriter.writeStartElement("reference");
							xmlStreamWriter.writeCharacters(String.valueOf(item.getReference().getId()));
							xmlStreamWriter.writeEndElement();
							xmlStreamWriter.writeStartElement("referencePath");
							xmlStreamWriter.writeCharacters(String.valueOf(item.getReference().getId()));
							xmlStreamWriter.writeEndElement();
						}
						
					 	for(MSColumn column:ams.getColumns()){
					 		xmlStreamWriter.writeStartElement(column.getHeading());
					 		Object myObj = item.getField(column.getHeading());
					 		if((myObj != null) && (myObj.toString() != null))
					 			xmlStreamWriter.writeCharacters(StringEscapeUtils.escapeXml(myObj.toString()));
					 		xmlStreamWriter.writeEndElement();
					 	}
				 		xmlStreamWriter.writeEndElement();
					}
		            ...
		            throw new RuntimeException("Could not obtain XML results for SQL: " +queryString, ex);
		        }finally {
		            if (xmlStreamWriter != null) {
		                try {
							xmlStreamWriter.close();
