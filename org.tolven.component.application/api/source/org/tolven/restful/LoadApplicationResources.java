...
public class LoadApplicationResources {
    ...
    protected RuleBaseLocal getRuleBaseBean() {
        if (ruleBaseBean == null) {
            String jndiName = "java:app/tolvenEJB/RuleBaseBean!org.tolven.rules.RuleBaseLocal";
            try {
                InitialContext ctx = new InitialContext();
                ruleBaseBean = (RuleBaseLocal) ctx.lookup(jndiName);
            } catch (Exception ex) {
                throw new RuntimeException("Could not lookup " + jndiName);
            }
        }
