...
public class GraphMenuEventHandler extends MenuEventHandler {
    ...
    public void displayGraph() throws Exception {
        ...
        int width = 600;
        int height = 400;
        String filename = ServletUtilities.saveChartAsPNG(chart, width, height, null);
    ...
    private JFreeChart createChart() throws Exception {
        ...
        Range dateRange = axis.getRange();
        axis.setRange(Range.expand(dateRange, .2, .1)); //Expand the range for the graph by 20% on the lower end and 10% on the upper end.
        //axis.setDateFormatOverride(new SimpleDateFormat("MMM-yyyy hh:mm"));
        //CCHIT merge
    ...
    private XYDataset createDataset() throws Exception {
        ...
        for (MenuData md : menuData) {
            ...
            if (currentTimeSeries != null) {
                ...
                try{
                Double valueAsDouble = Double.valueOf(md.getStringField(valueColumn));
                currentTimeSeries.addOrUpdate(hour,valueAsDouble);
                }
                catch(Exception e)
                {}
