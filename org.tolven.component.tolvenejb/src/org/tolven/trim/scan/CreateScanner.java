...
public class CreateScanner extends BindScanner {
	...
	private static final Logger logger = Logger.getLogger(CreateScanner.class);
...
//		getMenuBean().populateMenuData( sourceMap, mdPlaceholder );
	...
	public MenuData bindToPlaceholder( BindRequest request  ) {
		logger.debug( "Bind to placeholder at: " + request.location);
		MenuData md = null;
...
//		}
	...
	public void populateBindMaps(BindRequest request, MenuData md) {
		...
		for (BindMap mp : request.placeholderBind.getMaps()) {
			...
			if(bindName  != null) {
				logger.debug( "in CreateScanner.bindToPlaceholder: map(name, path):" + bindName + ", " + bindPath);
