...
@Stateless
@Local(MdvQueueLocal.class)
public class MdvQueueBean implements MdvQueueLocal {
    ...
    @Resource(name = "queue/mdv")
    private Queue mdvQueue;

    @Override
    public String getQueueName() {
        if (queueName == null) {
            try {
                queueName = mdvQueue.getQueueName();
    ...
    public void send(List<Serializable> payloads, List<Map<String, Object>> listOfProperties) {
        ...
        try {
            ...
            MessageProducer messageProducer = session.createProducer(mdvQueue);
