...
public class TolvenMessageScheduler implements TolvenMessageSchedulerLocal {
    ...
    @Override
    public void delete(TolvenMessage tm) {
        long tmId = tm.getId();
        Query query = em.createQuery("DELETE FROM TolvenMessageAttachment tma WHERE tma.parent.id = :id");
        query.setParameter("id", tmId);
        query.executeUpdate();
        query = em.createQuery("DELETE FROM TolvenMessage tm WHERE tm.id = :id");
        query.setParameter("id", tmId);
        query.executeUpdate();
    }
    ...
    public void schedule(List<TolvenMessage> tms, List<Map<String, Object>> listOfProperties, Date queueOnDate) {
        ...
        for (int i = 0; i < tms.size(); i++) {
            ...
            if (queueOnDate != null) {
                ...
                logger.debug("TolvenMessage " + tm.getId() + " scheduled for " + ruleQueueBean.getQueueName() + " at " + queueOnDate);
            }
        }
        if (queueOnDate == null) {
    ...
    public void timeout(Timer timer) {
        //logger.debug("TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT-TIMEOUT");
    ...
    private boolean queueMessages() {
        Date now = new Date();
        Query query = em.createQuery("SELECT tm FROM TolvenMessage tm WHERE " + "tm.scheduleDate is NOT NULL AND " + "(tm.queueOnDate is NULL or tm.queueOnDate <= :now) AND " + "tm.queueDate is NULL");
    ...
    private boolean queueMessages(List<TolvenMessage> tms, Date now) {
        if (!tms.isEmpty()) {
            //logger.debug("SCHEDULE_MESSAGES-SCHEDULE_MESSAGES-SCHEDULE_MESSAGES-SCHEDULE_MESSAGES-SCHEDULE_MESSAGES");
            for (TolvenMessage tm : tms) {
                ...
                tm.setQueueDate(now);
                logger.debug("TolvenMessage " + tm.getId() + " queued to " + ruleQueueBean.getQueueName() + " at " + now);
            }
        }
